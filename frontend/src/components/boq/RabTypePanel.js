        ...
        <TableBody>
          {rows.map((r) => (
            <TableRow key={r.ref_code} data-testid={isType ? P80.typeRow : P80.addonRow} data-code={r.ref_code}>
              ...
              <TableCell className="text-right tabular-nums font-medium">{formatIDR(r.total)}</TableCell>
...
export default function RabTypePanel({ onChanged }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <RabTemplateDialog kind={edit?.kind} target={edit?.target} open={!!edit} onOpenChange={(v) => !v && setEdit(null)}
