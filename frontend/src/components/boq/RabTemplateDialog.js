...
export default function RabTemplateDialog({ kind, target, open, onOpenChange, onDone }) {
  ...
  useEffect(() => {
    ...
    api.get(`/rab/templates/${kind}/${encodeURIComponent(target.ref_code)}`)
      .then((r) => setRows((r.data.data.items || []).map((it) => ({ ...it, qty: String(it.qty), unit_price: String(it.unit_price), step_code: it.step_code || "" }))))
      .catch(() => setRows([]));
    if (kind === "unit_type") {
