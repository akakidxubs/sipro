...
async def save_template(org: str, kind: str, ref_code: str, items: list, actor: str) -> dict:
    ...
    await db.rab_templates.update_one(
        {"org_id": org, "kind": kind, "ref_code": ref_code},
        {"$set": {"items": norm, "total": sum(i["amount"] for i in norm), "updated_by": actor, "updated_at": ts},
